...
class Registrations {
	...
	private function render_toolbar( $event_id ) {
		...
		if ( current_user_can( 'edit_post', $event_id ) ) {
			...
			// USCF status validation: only when Tournament Manager (which
			// owns the admin-ajax handler and the JS wiring this button -
			// its WPMTM_USCF_Status class) is active and the current user
			// can manage tournaments there. Same class_exists() guard style
			// as the import button below, so ETR keeps working unchanged
			// when Tournament Manager is absent or deactivated.
			if ( class_exists( '\WPMTM_USCF_Status' ) && current_user_can( 'wpmtm_manage_tournaments' ) ) {
				printf(
					' <button type="button" class="etr-btn" data-wpmtm-validate-players data-event="%d" data-nonce="%s">%s</button>',
					(int) $event_id,
					esc_attr( wp_create_nonce( 'wpmtm_validate_players_' . $event_id ) ),
					esc_html__( 'Validate players', 'etr' )
				);
			}
