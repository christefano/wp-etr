...
When the [Tournament Manager](https://github.com/christefano/tournament-plugin) plugin is active, users who can manage tournaments there also see an "Import to Tournament Manager" button next to "Pairing export". It creates (or reuses) the tournament linked to the event and hands the roster straight to Tournament Manager's import preview screen - no CSV download/upload round trip needed.
...
ETR's version number mirrors the versions of Event Tickets Extra Custom Fields that ETR is known to work with. For example, ETR v5.2.4 is known to work with Event Tickets Extra Custom Fields v5.2.4 and may not have been tested with newer versions.
