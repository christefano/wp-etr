...
class Plugin {
	...
	public function build_sections( $event_id ) {
		...
		foreach ( $this->get_event_attendees( $event_id ) as $aid ) {
			...
			$buckets[ $section ][] = [
				...
				// Tournament Manager family-avoidance (docs/SPEC.md,
				// 2026-07-14, wp-tournament-manager): a raw, untrimmed-by-
				// nothing-more-than-trim() passthrough of the attendee's
				// ETECF "Parent email" field, read directly the same way
				// attendee_photo_id() reads the photo field - literal key,
				// not part of the configurable field mapping (ETECF only
				// ever has one such field for this club's registration
				// form), consumed by WPMTM_Admin_Import::build_rows_from_event().
				'parent_email'   => $this->attendee_parent_email( $aid ),
			];
		}
	...
	/**
	 * An attendee's ETECF "Parent email" field, trimmed, or '' when unset.
	 * Consumed by wp-tournament-manager's roster import for best-effort
	 * family-avoidance in Suggest pairings; reads the literal
	 * `etecf_parent_email` meta key directly (not a configurable field
	 * mapping - see build_sections()'s own comment on this key), the same
	 * decoupled-from-ETECF's-PHP-classes approach attendee_photo_id() above
	 * uses.
	 */
	private function attendee_parent_email( $aid ) {
		return trim( (string) get_post_meta( (int) $aid, 'etecf_parent_email', true ) );
	}
